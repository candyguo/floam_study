...
void laser_processing(){
    while(1){
        if(!pointCloudBuf.empty()){
            ...
            //对三种点云进行发布，其中filtered pointcloud是slam中用到的点云，是边缘点云和平面点云的总和
...
//每一个ros node是一个执行的节点，相当于同时在执行的进程
int main(int argc, char **argv)
{
    ...
    //get param from parameters in launch file
    ...
    //订阅原始点云消息，开启激光处理线程，进行处理，发布filtered pointcloud, edge pointcloud and surf pointcloud
